...
def load_cypher_llm(LLM_CYPHER: str):
    if LLM_CYPHER == "gpt-4":
        ...
        return AzureChatOpenAI(temperature=0, model_name="gpt-4o", streaming=True,azure_endpoint = "https://openai-ml-emerson.openai.azure.com/",deployment_name="gpt-4o-emerson", openai_api_key="3faf06966d814857962860df63d6797f",openai_api_version="2024-05-01-preview")
...
st.markdown(
    ...
    <div style="display: flex; justify-content: center;">
        <img src="https://lh3.googleusercontent.com/d/1pANNU19GocDRHj9SQjgfMhWBx_gzhjs1" alt="Imagem" style="max-width: 100%; height: auto;">
    </div>
    """,
    unsafe_allow_html=True
)

#st.image("https://lh3.googleusercontent.com/d/1p9mneLS0Kxoeu5YlSctbQZvOundhnTfs")

#title_col, img_col = st.columns([2,2])

#with title_col:
 #   st.title("Cybersecurity LLM Chatbot")
#with img_col:
 #   st.image("https://d3k2f0s3vqqs9o.cloudfront.net/media/2BF7D1AD-4DB7-43DF-A1F1EE0C57549FC3/D44A2456-8560-4D0B-B86BF6C787C83E88/webimage-22505CAA-2FAE-439A-B92F4997D6A39DB8.png", width=150)
...
with st.expander("Sample Questions"):
    st.write("")
    st.write("- How many clients are there in the database?")
    st.write("- What are the ranking of WIFI devices by manufacturer in the database? Format in a Table.")
    st.write("- How many different community groups for everything related to client, ssid, wifi device, wifi bridge and ssids are there in the database today in total?")
    st.write("- How can I get the WIFI Device's canonical name, device key, manufacturer, last BSSID it was connected to, signal strength, total snapshots of the device (including first and last), and date of the last snapshot, sorted by the timestamp of the last snapshot?")
...
if user_input:
    with st.spinner("Hmm... Neo4j, is that you whispering the answer? ... Let me Check.... "):
        ...
        try:
            ...
            st.write("Failed to answer question. Please try again.")
            print(e)

    st.write(f"Elapsed Time: {timer() - start:.2f}s")
    ...
    with col1:
        if "system_msgs" in st.session_state and st.session_state["system_msgs"]:
            for i in range(len(st.session_state["system_msgs"]) - 1, -1, -1):
                st.write("Assistant: ")
                st.write(st.session_state["system_msgs"][i],key=str(i))
    ...
    with col3:
        if database_results:
            st.text_area(" Last results from Neo4j DB", database_results, key="_database", height=240)
